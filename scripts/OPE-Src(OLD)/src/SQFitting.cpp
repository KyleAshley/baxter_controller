...
double SQFitting::qualityOfFit(pcl::PointCloud<pcl::PointXYZ>& cloud, SQParameters& sqParams) {
	...
	for (int i = 0; i < (int) cloud.points.size(); i++) {
		...
		a = pow(fabs(static_cast<double>(_pnt(0, 0) / sqParams.a1)), 2.0 / sqParams.e2);
		b = pow(fabs(static_cast<double>(_pnt(1, 0) / sqParams.a2)), 2.0 / sqParams.e2);
		c = pow(fabs(static_cast<double>(_pnt(2, 0) / sqParams.a3)), 2.0 / sqParams.e1);
