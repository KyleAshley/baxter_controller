...
double Minimization::errorFunc(const pcl::PointCloud<pcl::PointXYZ>& cloud, SQParameters& sqParams) {
	...
	for (int i = 0; i < (int) cloud.points.size(); i++) {
		...
		Eigen::MatrixXd pntTrans(3, 1);

		pntMove(0, 0) = static_cast<double> (cloud.points[i].x - sqParams.px);
		pntMove(1, 0) = static_cast<double> (cloud.points[i].y - sqParams.py);
		pntMove(2, 0) = static_cast<double> (cloud.points[i].z - sqParams.pz);

		pntTrans = mInv * pntMove;

		pntTrans(0, 0) = pntTrans(0, 0) / (sqParams.kx * pntTrans(2, 0) / sqParams.a3 + 1);
		pntTrans(1, 0) = pntTrans(1, 0) / (sqParams.ky * pntTrans(2, 0) / sqParams.a3 + 1);

		a = pow(fabs(static_cast<double>(pntTrans(0, 0) / sqParams.a1)), 2.0 / sqParams.e2);
		b = pow(fabs(static_cast<double>(pntTrans(1, 0) / sqParams.a2)), 2.0 / sqParams.e2);
		c = pow(fabs(static_cast<double>(pntTrans(2, 0) / sqParams.a3)), 2.0 / sqParams.e1);
		d = pow(a + b, static_cast<double>(sqParams.e2 / sqParams.e1));
		e = d + c;
		f = pow(e, static_cast<double>(sqParams.e1));
